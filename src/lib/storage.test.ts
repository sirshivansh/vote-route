...
describe("Storage Library", () => {
  ...
  it("should handle corrupted localStorage data gracefully", () => {
    localStorageMock.setItem("vja:profile", "invalid json{{{");
    expect(getProfile()).toBeNull();
  });

  it("should handle corrupted completed steps data gracefully", () => {
    localStorageMock.setItem("vja:completed-steps", "not-an-array!!!");
    expect(getCompleted()).toEqual([]);
  });
});
