...
import { FIRST_TIME_VOTER_JOURNEY, calcReadiness, readinessLabel, getNextStep } from "@/lib/journey";
...
export const Route = createFileRoute("/readiness")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your personalised readiness report — see what's complete, what's missing, and recommended next actions." },
    ],
  }),
  ...
  return (
    <PageShell crumbs={[{ label: "Dashboard", to: "/dashboard" }, { label: "Readiness" }]}>
      <header className="rounded-3xl border border-border bg-card p-6 sm:p-8 shadow-soft">
        <div className="grid gap-6 md:grid-cols-[auto_1fr] md:items-center">
          <div className="flex justify-center md:justify-start">
            <ReadinessRing score={score} size={170} stroke={14} label="Readiness" sublabel={meta.label} />
          </div>
          <div>
        ...
        <section className="rounded-2xl border border-leaf/30 bg-leaf/5 p-5 sm:p-6">
          ...
          {doneSteps.length === 0 ? (
            ...
            <ul className="mt-3 space-y-2">
              {doneSteps.map((s) => (
                <li key={s.id} className="flex items-start gap-2 rounded-xl bg-card p-3 border border-border">
                  <Check className="h-4 w-4 text-leaf shrink-0 mt-0.5" />
                  <div className="min-w-0">
                    <Link to="/step/$stepId" params={{ stepId: s.id }} className="text-sm font-medium hover:underline truncate block">
                      {s.title}
                    </Link>
            ...
            <ul className="mt-3 space-y-2">
              {missingSteps.map((s) => (
                <li key={s.id} className="rounded-xl bg-card p-3 border border-border">
                  <Link to="/step/$stepId" params={{ stepId: s.id }} className="text-sm font-medium hover:underline block">
                    {s.title}
                  </Link>
      ...
      <section className="mt-6 rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-soft">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Smart recommendations</h2>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <Recommendation
            title={score < 30 ? "Start with the basics" : score < 80 ? "Keep momentum" : "Almost there"}
            body={
              nextStep
                ? `Open "${nextStep.title}" — it's ${nextStep.estimate} and adds ${nextStep.weight}% to your readiness.`
                : "You've completed every step!"
            }
            cta={nextStep ? { label: "Open step", to: `/step/${nextStep.id}` } : { label: "View completion", to: "/done" }}
          />
          <Recommendation
