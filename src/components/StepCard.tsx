...
export const StepCard = React.memo(function StepCard({
  ...
  return (
    <div
      className={cn(
        "group relative flex gap-4 rounded-2xl border bg-card p-5 transition-all",
        isActive
          ? "border-primary/50 shadow-glow"
          : isDone
            ? "border-leaf/30"
            : "border-border shadow-soft hover:border-primary/30 hover:-translate-y-0.5",
      )}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isDone && (
            <span className="ml-auto inline-flex items-center gap-1 text-xs font-medium text-leaf">
              <Check className="h-3 w-3" /> {t("common:status.done")}
            </span>
          )}
        </div>
        ...
        <h3
          className={cn(
            "mt-2 text-base sm:text-lg font-semibold tracking-tight break-words",
        ...
        {!compact && (
          <p className="mt-1 text-sm text-muted-foreground leading-relaxed">{step.shortDesc}</p>
        )}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {!isDone && onComplete && (
            <button
              onClick={onComplete}
              className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-4 py-2 text-xs font-medium text-foreground hover:bg-muted transition-colors"
            >
