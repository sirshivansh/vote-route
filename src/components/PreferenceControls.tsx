...
function PreferenceRow({
  ...
  return (
    <button
      type="button"
      onClick={onChange}
      className="flex w-full items-start justify-between gap-3 rounded-xl border border-border bg-background p-4 text-left hover:border-primary/30"
    >
      ...
      <span
        className={`mt-0.5 inline-flex h-5 w-9 shrink-0 rounded-full p-0.5 transition-colors ${checked ? "bg-primary" : "bg-muted"}`}
      >
        <span
          className={`h-4 w-4 rounded-full bg-card transition-transform ${checked ? "translate-x-4" : "translate-x-0"}`}
        />
      </span>
    </button>
  );
